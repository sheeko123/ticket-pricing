...
def analyze_price_distribution(df):
    ...
    plt.tight_layout()
    plt.savefig('graphs/price_analysis_groupcv.png')
    print("Saved price analysis visualizations to 'graphs/price_analysis_groupcv.png'")
...
def build_glm_model_groupcv(df_model, model_type='tweedie'):
    """
    Build and evaluate GLM model with grouped cross-validation
    """
    print(f"\nBuilding GLM model with {model_type} distribution using grouped cross-validation...")
    ...
    print("\nPerforming grouped cross-validation...")
    for fold, (train_index, test_index) in enumerate(group_kfold.split(X, y, groups=groups), 1):
        print(f"\nFold {fold}")
    ...
    plt.tight_layout()
    plt.savefig(f'graphs/price_prediction_{model_type}_groupcv.png')
    print(f"Saved price prediction plot to 'graphs/price_prediction_{model_type}_groupcv.png'")
...
def main():
    ...
    gamma_results_floor, X_test_floor, y_test_floor = build_glm_model_groupcv(df_model_floor, model_type='gamma')
    tweedie_results_floor, _, _ = build_glm_model_groupcv(df_model_floor, model_type='tweedie')
    ...
    gamma_results_non_floor, X_test_non_floor, y_test_non_floor = build_glm_model_groupcv(df_model_non_floor, model_type='gamma')
    tweedie_results_non_floor, _, _ = build_glm_model_groupcv(df_model_non_floor, model_type='tweedie')
